...
!     limit the rate of H2 formation on grains (the molecule cannot desorb from the surface)
      IF(PDR(P)%Tdust.LT.10.0D0) THEN
         PDR(P)%Tdust=10.0D0
      END IF

!     Check that the dust temperature is physical
      IF(PDR(P)%Tdust.GT.1000) THEN
