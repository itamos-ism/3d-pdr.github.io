...
#ifdef RAYTHEIA
#ifdef OPENMP
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(p, j, box1, thfpix, phfpix, ray)
#endif
do p=1,pdr_ptot
   box1%min = corner_min
   box1%max = corner_max
   ...
   do j=0,nrays-1
      ...
      call raytheia_table(ray, box1, levels-1, p, j)
   enddo
enddo
#ifdef OPENMP
!$OMP END PARALLEL DO
...
do p=1,pdr_ptot
    ...
    allocate(ra(0:pdr_ptot-1)) !needs one extra place for sorting in heapsort
    allocate(rb(1:pdr_ptot-1)) !-1 to avoid overlapping origin & pdrpoint
    ...
    enddo
    ktot=kk !ktot should be pdr_ptot-1
    if (ktot.ne.(pdr_ptot-1)) then 
      write(6,*) 'ktot = ',ktot,' pdr_ptot-1 = ',pdr_ptot-1
      stop 'ktot is not equal to pdr_ptot-1 !!'
    endif
    ...
    ep=0.
    do j=0,nrays-1
      pdr(p)%epoint(1:3,j,0) = origin(1:3)
    end do
...
!       !updates memory and stores the evaluation point in the original computational domain (so evaluation point + origin)
       pdr(p)%epray(ipix) = pdr(p)%epray(ipix)+1
       id = pdr(p)%epray(ipix)
       if (pdr(p)%epray(ipix).gt.maxpoints) STOP 'Increase maxpoints!'
       pdr(p)%epoint(1:3,ipix,id)=ep(1:3,ipix)+origin(1:3)
       pdr(p)%projected(ipix,id)=rb(k)
