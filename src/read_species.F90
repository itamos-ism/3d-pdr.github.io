...
use healpix_types
use global_module
use maincode_module, only: coolant,coo
      ...
      INTEGER(kind=i4b) :: I,INDEX,SPECIESFILE,J
...
!C     error and the code will crash. This is a useful bug catch.
      ...
      OPEN(SPECIESFILE,FILE="chemfiles/species_"//trim(adjustl(suffix)),STATUS="OLD")
      REWIND(SPECIESFILE)
      DO I=1,NSPEC
...
!C        Assign the various index labels to their correct species.
         ...
         IF(SPECIES(I).EQ."CN        ") NCN     = I
         IF(SPECIES(I).EQ."C2H       ") NC2H    = I
         IF(SPECIES(I).EQ."OH        ") NOH     = I
         IF(SPECIES(I).EQ."OH+       ") NOHx    = I
         ...
         IF(SPECIES(I).EQ."HCN       ") NHCN    = I
         IF(SPECIES(I).EQ."HNC       ") NHNC    = I
...
!C     specified, as the sum of the metal ion abundances
      IF(ABUNDANCE(NELECT).LE.0.0D0) THEN
       ...
       IF(NNx.NE.0) ABUNDANCE(NELECT)=ABUNDANCE(NELECT)+ABUNDANCE(NNx)
