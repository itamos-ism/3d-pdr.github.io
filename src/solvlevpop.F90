SUBROUTINE solvlevpop(NLEV,TRANSITION,density,SOLUTION)
...
!        Replace negative level populations due to numerical noise around 0
         DO I=1,NLEV
            if (solution(i).lt.0.0D0) solution(i)=0.0D0!1.0D-99
         ENDDO
...
!   SUBROUTINE GAUSS_JORDAN_writes(A,N,NP,B,coolant,ill)
   SUBROUTINE GAUSS_JORDAN_writes(A,N,NP,B,ill)
      ...
      use definitions
      use healpix_types
      use maincode_module, only : p,pdr
...
!                     write(6,*) 'grid point = ',p, ' coolant = ',coolant
                     write(6,*) 'gastemperature = ',pdr(p)%nTgas
                     STOP
                  ENDIF !IPIV(K).EQ.0
               ENDDO !K=1,N
            ENDIF !IPIV(J).NE.1
